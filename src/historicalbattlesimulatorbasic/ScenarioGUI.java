...
public class ScenarioGUI implements MouseListener
{
//    JLabel bgL = new JLabel();
  ...
  //initualize ScenarioGUI whenever need to have a new Panel with mouselistener (only called once i think)
   public ScenarioGUI(JPanel panel) {
       ScenarioGUI.panel=panel;
       ScenarioGUI.panel.addMouseListener(this);
       ScenarioGUI.statPanel.setVisible(false);
   ...
   private static void initCombatPanel(){
       ...
       combatPanel2.setBounds(ScenarioGUI.gameFrame.getWidth()/2-100,ScenarioGUI.gameFrame.getHeight()-4*ScenarioGUI.gameFrame.getHeight()/5,300,200);
       ScenarioGUI.panel.add(combatPanel2);
       ScenarioGUI.repainter();
   ...
   private static void initFormPanel()  {
         ...
         formationPanel.setBounds(ScenarioGUI.buttonPanel.getBounds());
         ...
         Component c = ScenarioGUI.buttonPanel.getComponent(0);
...
//         busy=true;
         button[0].addActionListener(new ActionListener() {
           ...
           {
               ScenarioGUI.unitSelected.expendUnitPoint();
               ScenarioGUI.removeSoldiersFromPreviousTiles(ScenarioGUI.unitSelected);
               ScenarioGUI.unitSelected.currentFormation.rowFormation();
               ScenarioGUI.toggleButtons(formationPanel, false);
               ScenarioGUI.toggleButtons(buttonPanel, true);
          }
       });
       ...
       button[1].addActionListener(new ActionListener() {
           ...
           public void actionPerformed(ActionEvent ae) //box Formation
           {
               ScenarioGUI.unitSelected.expendUnitPoint();
               ScenarioGUI.removeSoldiersFromPreviousTiles(ScenarioGUI.unitSelected);
               ScenarioGUI.unitSelected.currentFormation.setBoxFormation();
...
//           {
//               
//               ScenarioGUI.unitSelected.currentFormation.setWedgeFormation();
//               ScenarioGUI.toggleButtons(formationPanel, false);
//               ScenarioGUI.toggleButtons(buttonPanel, true);
//
//           }
//       });
       button[3].addActionListener(new ActionListener(){
           ...
           public void actionPerformed(ActionEvent ae){
               ...
               ScenarioGUI.unitSelected.expendUnitPoint();
               ScenarioGUI.removeSoldiersFromPreviousTiles(ScenarioGUI.unitSelected);
               ScenarioGUI.unitSelected.currentFormation.setColumnFormation();
               ScenarioGUI.toggleButtons(formationPanel, false);
               ScenarioGUI.toggleButtons(buttonPanel, true);
   ...
   public static Tile getTileClicked() {
         if(ScenarioGUI.tileClicked!=null)
            return ScenarioGUI.tileClicked;
        else
            return null;
    }
...
//        Unit unit= UnitLoader.allUnits.get(ScenarioGUI.unitNum-1);
        //sets the tile location of where they are at
        unit.setPosition(ScenarioGUI.tileClicked.xPosition,ScenarioGUI.tileClicked.yPosition);
        //adds it to the arrayList in ScenarioGUI that is currently storing the unitDraws
        ScenarioGUI.unitDraws.add(new UnitDraw(unit));
        
        
        ScenarioGUI.tileClicked=null; //used to avoid potential errors, tileClicked is reloaded everytime a tile is clicked on
...
//        ScenarioGUI.repainter(); //repaints
    }
   ...
   public void mouseClicked(MouseEvent mac) {
       double findTileX= Math.ceil(mac.getX()/ScenarioGUI.tileWidth);
       double findTileY=Math.ceil(mac.getY()/ScenarioGUI.tileWidth);
       ScenarioGUI.tileClicked=ScenarioGUI.tileGameMap[(int)findTileX][(int)findTileY]; //sets the tile= the tile with the coords in the tileGameMap
...
//       if(ScenarioGUI.tileClicked!=null)
        System.out.println("is this tile blocked " + ScenarioGUI.tileClicked.tileBlocked);
              ...
              if(player1Turn())   
              {
                  for(int i=0;i<Game.playersForDemo.get(0).allUnits.size();i++)
                  {
                      if(player1UnitIsEqualToUnitSelectedAt(i))
                      {
                        ...
                        //this is where unitselected gets initialized.  it will stay initialized until cancel selection is pressed
                          ScenarioGUI.unitSelected=Game.playersForDemo.get(0).allUnits.get(i);
                          toggleButtons(ScenarioGUI.buttonPanel,true);
                      }
                  }
                  for(int i=0;i<Game.playersForDemo.get(1).allUnits.size();i++)
                  {
                      if(player2UnitIsEqualToUnitSelectedAt(i))
                      {
                          ...
                          {
                               ScenarioGUI.enemySelected = Game.playersForDemo.get(1).allUnits.get(i);
                               ScenarioGUI.printStats(Game.playersForDemo.get(1).allUnits.get(i));
                          }
                      }
                  }
              }
               else if(player2Turn())
               {
                   for(int i=0;i<Game.playersForDemo.get(1).allUnits.size();i++)
                   {
                      if(player2UnitIsEqualToUnitSelectedAt(i))
                        {
                            System.out.println("unitSelected is being initialized ");
                            ScenarioGUI.unitSelected=Game.playersForDemo.get(1).allUnits.get(i);
                            toggleButtons(ScenarioGUI.buttonPanel,true);
                        } 
                   }
                   for(int i=0;i<Game.playersForDemo.get(0).allUnits.size();i++)
                   {
                      if(player1UnitIsEqualToUnitSelectedAt(i))
                      {
                          ...
                          {
                              ScenarioGUI.statPanel=null;
                              enemySelected=null;
                          }
                          else
                          {
                               ScenarioGUI.enemySelected = Game.playersForDemo.get(0).allUnits.get(0);
                               ScenarioGUI.printStats(Game.playersForDemo.get(0).allUnits.get(0));
                          } 
                      }
                   }
               }
   ...
   public boolean thereIsAUnitReadyToBeLoaded() {
        ...
        return ScenarioGUI.tileClicked!=null&&unitNum!=0;
    }
    ...
    public static void initMoveCountPanel() {
        ...
        JLabel text = new JLabel(Integer.toString((int)ScenarioGUI.unitSelected.moveMentCounter));
        moveCountPanel.setBounds(ScenarioGUI.gameFrame.getWidth()/2,ScenarioGUI.gameFrame.getHeight()/2,
                ScenarioGUI.gameFrame.getWidth()/15,ScenarioGUI.gameFrame.getHeight()/15);
        ...
        ScenarioGUI.panel.add(moveCountPanel);
   ...
   public static void buttonLoader(){
       ...
       ScenarioGUI.buttonPanel.setLayout(null);
       buttonPanel.setBounds(0, ScenarioGUI.gameFrame.getHeight()-150,ScenarioGUI.gameFrame.getWidth(), 150);
       ...
       button[0].addActionListener(new ActionListener() {
           ...
           public void actionPerformed(ActionEvent ae) 
           {
               if(ScenarioGUI.unitSelected.hasUnitPoints())
               {
               ScenarioGUI.attackUnit=ScenarioGUI.unitSelected;
               ScenarioGUI.toggleButtons(ScenarioGUI.buttonPanel,false);
               ScenarioGUI.initCombatPanel();
               }
               else
                   {
                       JOptionPane.showMessageDialog(formationPanel, ScenarioGUI.unitSelected.nameOfUnit+" is out of unit points. Select a different unit, or end your turn" );
                       ScenarioGUI.toggleButtons(ScenarioGUI.buttonPanel,false);
                       ScenarioGUI.unitSelected=null;
                   }
           } 
       });
       button[1].addActionListener(new ActionListener() {
           ...
           public void actionPerformed(ActionEvent ae)
           {
              ...
              ScenarioGUI.toggleButtons(ScenarioGUI.buttonPanel,false);
               ...
               if(ScenarioGUI.attackUnit==null)
               {
...
//                        ScenarioGUI.initDefenseButton();
                  //elseIf(there is a specialAbility and the button has been intialized
                        ScenarioGUI.panel.getComponentAt(ScenarioGUI.buttonPanel.getComponent(2).getLocation()).setVisible(true);
                   ScenarioGUI.unitSelected.brace();
               } 
           }
               else
                   {
      ...
      //if statPanel!=null (should be never) and is not visible, call method print stats
               if(componentNotNullAndIsNotVisible(statPanel))
                    ScenarioGUI.printStats(ScenarioGUI.unitSelected);
       ...
       button[3].addActionListener(new ActionListener() {
           ...
           public void actionPerformed(ActionEvent ae) 
           {
                  ...
                  if(ScenarioGUI.moveC==null) //if the compass has yet to be initalized the first time, init it
                  {
                     ScenarioGUI.initializeCompass();
                  }
                  else if(!moveC.isVisible())
                  {
                     ScenarioGUI.moveC.setVisible(true);      
                  }
                  ScenarioGUI.initMoveCountPanel();
                  ScenarioGUI.repainter();
           ...
           public void actionPerformed(ActionEvent ae) 
           {
                   ...
                   if(ScenarioGUI.unitSelected.hasUnitPoints())
               {
                   ...
                   if(!formationPanel.isVisible())
                   {
                    ...
                    ScenarioGUI.toggleButtons(buttonPanel, false);
       ...
       button[5].addActionListener(new ActionListener() 
       {
           ...
           {
               //deselect Unit 
               if(ScenarioGUI.unitSelected!=null)
               {
                   ScenarioGUI.unitSelected=null;
                   ScenarioGUI.toggleButtons(ScenarioGUI.buttonPanel,false);
                   ScenarioGUI.statPanel.setVisible(false);
                   if(componentNotNullAndIsVisible(moveC))
                        ScenarioGUI.moveC.setVisible(false);
                   if(componentNotNullAndIsVisible(formationPanel))
                   {
                       formationPanel.setVisible(false);
                       toggleButtons(formationPanel,false);
                   }
                   if(ScenarioGUI.moveCountPanel!=null)
                   {
                       ScenarioGUI.moveCountPanel.setVisible(false);
                       ScenarioGUI.moveCountPanel=null;
                   }
               }
               else
               {
       ...
       endTurn.addActionListener(new ActionListener()
       {
             ...
             { 
                 ...
                 ScenarioGUI.refreshTurnPanel();
                 if(moveCountPanel!=null)
                     ScenarioGUI.moveCountPanel.setVisible(false);
                 
                 if(ScenarioGUI.formationPanel!=null&&formationPanel.isVisible())
                     ScenarioGUI.toggleButtons(formationPanel, false);
...
//                 System.out.println("test for end Turn Button");
   ...
   public static void addButtonsToPanel(JButton[] button) {
       ...
       ScenarioGUI.panel.add(buttonPanel);
       ScenarioGUI.repainter();
    }
   ...
   public static JButton[] initializeButtons(JButton[] button){
      ...
      button=getButtonInit(button,buttonImages);
       ...
       ScenarioGUI.endTurn=new JButton(end);
     ...
     //  endTurn.setContentAreaFilled(false);
       ...
       ScenarioGUI.initTurnPanel();
       endTurn.setBounds(ScenarioGUI.panel.getWidth()-100,turnPanel.getHeight(),100,50);
       ScenarioGUI.panel.add(endTurn);
   ...
   public static void initTurnPanel(){
     // ScenarioGUI.panel.remove(turnPanel);
       turnPanel = new JPanel();
       turnPanel.setBackground(Color.black);
       turnPanel.setBounds(ScenarioGUI.panel.getWidth()-100, 0, 100, 500);
    
        refreshTurnPanel();
       javax.swing.border.Border borderUsed = BorderFactory.createLineBorder(Color.white);
       turnPanel.setBorder(borderUsed);
       
       ScenarioGUI.panel.add(turnPanel);
       refreshTurnPanel();
...
//            {
//                JOptionPane.showMessageDialog(ScenarioGUI.panel,"<html><i>The 'God King' Xerxes of Persia stands on the verge of conquering all of Greece. <br>"
...
//            {
//                JOptionPane.showMessageDialog(ScenarioGUI.panel,"<html><i>After assault by the Persian forces, the first day of fighting draws to a close...</i></html>");
//                oneDisplay2 = false;
//            }
//            else if(turnCountForPersians==10&&oneDisplay3)
//            {
//                JOptionPane.showMessageDialog(ScenarioGUI.panel,"<html><i>After the failure of Xerxes deadly 'Immortals' to encircle and destroy the Spartan forces <br"
...
//            {
//                JOptionPane.showMessageDialog(ScenarioGUI.panel,"<html><i>Although all the Spartans and Thebans who remained perished, they held the pass for long enough to allow the <br>"
//                        + "rest of Greece to prepare for war. Herodatus, classical historian remarks<br>"
//                        + "</i></html>");
//                JOptionPane.showMessageDialog(ScenarioGUI.panel,"<html><i>'Go tell the Spartans, passerby:\n" +
...
//        double xPos = unitSelected.tileOccupied.xPosition;
  //      double yPos = unitSelected.tileOccupied.yPosition;
       ...
       // close.setBounds(ScenarioGUI.statPanel.getWidth(), -4*ScenarioGUI.statPanel.getHeight()/5, ScenarioGUI.statPanel.getWidth(), 50);
        ...
        ScenarioGUI.statPanel.add(nameSection);
        ...
        ScenarioGUI.statPanel.add(stats);
       ...
       //add image of unit selected
        //ScenarioGUI.statPanel.add(up);
        //ScenarioGUI.statPanel.add(new JLabel(" "));
        ScenarioGUI.statPanel.add(close);
        //set the location and size of panel
        ScenarioGUI.statPanel.setBounds(statPanel.getWidth(),200/*(ScenarioGUI.turnPanel.getHeight()+ScenarioGUI.endTurn.getHeight())*/,200,300);
        //set the panel to true
        ScenarioGUI.statPanel.setVisible(true);
        //add the panel to the big panel
        ScenarioGUI.panel.add(ScenarioGUI.statPanel);
        ScenarioGUI.repainter();
    }
   ...
   public static boolean repainter(){
      if(ScenarioGUI.panel!=null&&ScenarioGUI.gameFrame!=null)
      {
         ScenarioGUI.panel.repaint();
         ScenarioGUI.gameFrame.repaint();
         ScenarioGUI.gameFrame.revalidate(); 
         return true;
      }
      else
          return false;
   ...
   private static void initializeCompass() {
           ...
           ScenarioGUI.moveC=new Compass();
           moveC.init(); //initialize the compass
           
           //set where the compass is going to be on the panel
           ScenarioGUI.moveC.setBounds(ScenarioGUI.gameFrame.getWidth()-200,ScenarioGUI.gameFrame.getHeight()-500,200,200);
           
           ScenarioGUI.moveC.setVisible(true);
           ScenarioGUI.panel.add(ScenarioGUI.moveC); //add it to the panel
    }
   ...
   public static void printStatsUpdater(Unit unit){
       if(ScenarioGUI.statPanel!=null&&ScenarioGUI.statPanel.isVisible())
       {
           ScenarioGUI.printStats(unit); //should be equal to ScenarioGUI.unitSelected
       }
   }
   protected boolean userTriesToSelectUnitBeforeAllUnitsArePlaced() {
        return ScenarioGUI.tileClicked!=null&&ScenarioGUI.tileClicked.isOccupied&&ScenarioGUI.unitSelected==null&&formationPanel==null&&(Game.playersForDemo.get(1).up.numOfUnitsToPlace>0||Game.playersForDemo.get(0).up.numOfUnitsToPlace>0);
    }
   ...
   protected boolean PlayerSelectedSameEnemyUnitAgain(int playerNum,int i) {
        return ScenarioGUI.enemySelected!=null&&ScenarioGUI.enemySelected==Game.playersForDemo.get(playerNum).allUnits.get(i);
    }
   protected boolean player2UnitIsEqualToUnitSelectedAt(int i) {
        return Game.playersForDemo.get(1).allUnits.get(i).getUnitID()==ScenarioGUI.tileClicked.getOccupier().getUnitID();
//        return Game.playersForDemo.get(1).allUnits.get(i).yPosition==ScenarioGUI.tileClicked.yPosition&&Game.playersForDemo.get(1).allUnits.get(i).xPosition==ScenarioGUI.tileClicked.xPosition;

    }
   protected boolean player1UnitIsEqualToUnitSelectedAt(int i) {
//        return Game.playersForDemo.get(0).allUnits.get(i).yPosition==ScenarioGUI.tileClicked.yPosition&&Game.playersForDemo.get(0).allUnits.get(i).xPosition==ScenarioGUI.tileClicked.xPosition;
        return Game.playersForDemo.get(0).allUnits.get(i).getUnitID()==ScenarioGUI.tileClicked.getOccupier().getUnitID();
    }
   protected boolean thereIsNoUnitCurrentlyAndThereIsAUnitOnThisTile() {
        return ScenarioGUI.tileClicked!=null&&ScenarioGUI.tileClicked.isOccupied&&ScenarioGUI.unitSelected==null;
        
//        return ScenarioGUI.tileClicked!=null&&ScenarioGUI.tileClicked.isOccupied&&!ScenarioGUI.impendingAttack&&ScenarioGUI.unitSelected==null&&formationPanel==null&&Game.playersForDemo.get(0).up.numOfUnitsToPlace==0&&Game.playersForDemo.get(1).up.numOfUnitsToPlace==0||(formationPanel!=null&&formationPanel.isVisible());
    }
   protected boolean player2IsReadyToLoadUnits() {
        return Game.playersForDemo!=null&&Game.playersForDemo.get(1).up.check&&ScenarioGUI.tileClicked!=null;
    }
   protected boolean player1IsReadyToLoadUnits() {
        return Game.playersForDemo!=null&&Game.playersForDemo.get(0).up.check&&ScenarioGUI.tileClicked!=null;
    }
     ...
     boolean secret=false;
       if(turnCountForPersians<20&&secret!=true)
       {
         //  System.out.println("TRUEEEE");
            ScenarioGUI.gameMap.generateTiles();
         try {
             ScenarioGUI.gameMap.saveMap("MapSaveTest");
   ...
   protected static int determineWhichUnitDrawContainsUnitIdEqaulToUnitSelectedAt(Unit u) {
        ...
        int index=-1;
        for(int i=0;i<ScenarioGUI.unitDraws.size();i++)
        {
//           System.out.println("in moveLogic before index selection");
            if(u.getUnitID()==ScenarioGUI.unitDraws.get(i).thisUnit.getUnitID())
            {
//                System.out.println("unitID of unitDraw at "+ i+ " = "+
//                        ScenarioGUI.unitDraws.get(i).thisUnit.getUnitID());
                index=i;
                break;
            }
        }if(index==-1)
   ...
   protected static void removeSoldiersFromPreviousTiles(Unit u) {
        ...
        for(int i=0;i<ScenarioGUI.numberOfTilesHeight;i++)
        {
           for(int j=0;j<ScenarioGUI.numberOfTilesWidth;j++)
            {
                if(thereIsASoldierWhereThereShouldNotBe(j, i,u))
                    ScenarioGUI.tileGameMap[j][i].removeSoldier();
            } 
        }
   ...
   protected static ArrayList<Point>  findSoldiersOfThisUnit(Unit unit){
       ...
       for(int i=0;i<ScenarioGUI.numberOfTilesHeight;i++)
       {
           for(int j=0;j<ScenarioGUI.numberOfTilesWidth;j++)
           {
               if(ScenarioGUI.tileGameMap[j][i].isOccupied&&ScenarioGUI.tileGameMap[j][i].getOccupier().getUnitID()==unit.getUnitID())
   ...
   protected static boolean thereIsASoldierWhereThereShouldNotBe(int j, int i,Unit u) {
        return ScenarioGUI.tileGameMap[j][i].getOccupier()!=null&&ScenarioGUI.tileGameMap[j][i].getOccupier().getUnitID()==u.getUnitID();
    }
